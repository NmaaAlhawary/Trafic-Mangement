...
inline function get_type_overload(const void *this_ptr, const detail::type_info *this_type, const char *name)  {
    ...
    if (frame) {
        ...
        if ((std::string) str(code->co_name) == name && code->co_argcount > 0) {
            ...
            PyObject *self_arg = PyTuple_GET_ITEM(varnames, 0);
            PyObject *self_caller = PyObject_GetItem(locals, self_arg);
            if (self_caller == self.ptr()) {
                Py_XDECREF(self_caller);
                Py_XDECREF(varnames);
                Py_XDECREF(locals);
                Py_XDECREF(code);
                Py_XDECREF(frame);
        ...
        Py_XDECREF(frame);
#else
        if ((std::string) str(frame->f_code->co_name) == name &&
