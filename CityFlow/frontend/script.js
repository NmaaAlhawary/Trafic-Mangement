...
async function loadRealMapDemo() {
    ...
    loading = true;
    try {
        roadnetData[0] = await loadTextResource("testdata/real_map_roadnet.json");
        document.getElementById("roadnet-label").innerText = "testdata/real_map_roadnet.json";
        startLoadedRoadnetOnly();
    } catch (e) {
        infoReset();
        infoAppend("Real map roadnet not found yet");
        infoAppend("Run tools/osm_to_cityflow.py first to build testdata/real_map_roadnet.json");
        console.error(e.message);
        loading = false;
    }
}
